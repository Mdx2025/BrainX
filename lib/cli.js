...
Commands:
  ...
  feedback --id <memory_id> (--useful | --useless | --doubtful | --incorrect)
      ...
      --incorrect Mark memory as obsolete and superseded
...
async function cmdRuntimeReport(args, deps = {}) {
  ...
  const [overall, byAgent, bySurface, topMems, worstAgents] = await Promise.all([
    dbApi.query(
      `SELECT
         COUNT(*)::int AS injections,
         SUM(selected_count)::int AS total_memories_injected,
         SUM(signal_gate_dropped)::int AS gate_dropped,
         SUM(near_dup_dropped)::int AS dup_dropped,
         ROUND(AVG(latency_ms)::numeric, 1) AS avg_latency_ms,
         COUNT(*) FILTER (WHERE scored_at IS NOT NULL)::int AS scored,
         ROUND(AVG(NULLIF(referenced_count, 0)::numeric / NULLIF(selected_count, 0)) * 100, 2) AS signal_ratio_pct
    ...
    dbApi.query(
      `SELECT
         ...
         COUNT(*)::int AS injections,
         SUM(selected_count)::int AS mems_injected,
         SUM(referenced_count)::int AS mems_referenced,
         ROUND(AVG(latency_ms)::numeric, 1) AS avg_latency_ms,
         ROUND(SUM(referenced_count)::numeric / NULLIF(SUM(selected_count), 0) * 100, 1) AS signal_ratio_pct
       FROM brainx_runtime_injections
       WHERE injected_at >= NOW() - make_interval(days => $1)
       GROUP BY 1
    ...
    dbApi.query(
      `SELECT
       ...
       GROUP BY 1
       HAVING COUNT(*) >= 5
       ORDER BY signal_ratio_pct ASC NULLS LAST
       LIMIT 10`,
      [days]
    )
  ]);
  ...
  console.log(`BrainX Runtime Report — últimos ${days}d`);
  console.log('─'.repeat(60));
  console.log(`Inyecciones totales: ${o.injections || 0}`);
  console.log(`Memorias inyectadas: ${o.total_memories_injected || 0}`);
  console.log(`Bloqueadas por signal gate: ${o.gate_dropped || 0}`);
  console.log(`Bloqueadas por anti-dup: ${o.dup_dropped || 0}`);
  console.log(`Latencia promedio: ${o.avg_latency_ms || 0}ms`);
  console.log(`Turnos evaluados (scored): ${o.scored || 0}`);
  console.log(`Signal ratio global: ${o.signal_ratio_pct != null ? o.signal_ratio_pct + '%' : '—'}`);
  console.log('');
  console.log('Por agente:');
  for (const r of payload.by_agent) {
    const ratio = r.signal_ratio_pct != null ? `${r.signal_ratio_pct}%` : '—';
    console.log(`  ${r.agent.padEnd(24)} inj=${String(r.injections).padStart(4)} mem=${String(r.mems_injected).padStart(4)} ref=${String(r.mems_referenced || 0).padStart(4)} ratio=${ratio.padStart(6)} lat=${r.avg_latency_ms}ms`);
  }
  console.log('');
  console.log('Por superficie:');
  for (const r of payload.by_surface) {
    console.log(`  ${r.surface.padEnd(16)} inj=${String(r.injections).padStart(4)} avgMem=${r.avg_mems} lat=${r.avg_latency_ms}ms`);
  }
  console.log('');
  console.log('Top 10 memorias más inyectadas:');
  for (const m of payload.top_injected_memories) {
    console.log(`  [#${m.mem_id}] imp=${m.importance} type=${m.type} inj=${m.injections} agents=${m.unique_agents}  ${m.preview}`);
  }
  if (payload.worst_signal_agents.length) {
    console.log('');
    console.log('Agentes con peor signal ratio (>=5 turnos scored):');
    for (const r of payload.worst_signal_agents) {
      console.log(`  ${r.agent.padEnd(24)} inj=${String(r.injections).padStart(4)} ratio=${r.signal_ratio_pct}%`);
    }
  }
  return 0;
}

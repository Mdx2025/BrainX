...
const DEFAULT_WEIGHTS = {
  relevance: 0.42,   // cosine similarity
  importance: 0.18,  // normalized importance (0-1)
  recency: 0.12,     // exponential decay: exp(-days/30)
  tier: 0.05,        // tier bonus (hot=1, warm=0.7, cold=0.4, archive=0.2)
  feedback: 0.03,    // feedback_score
  confidence: 0.08,  // confidence_score
  provenance: 0.07,  // source_kind reliability
  typeSafety: 0.05,  // fact/decision/gotcha favored over learning/note
  verification: 0.10 // verified memories strongly preferred over hypothesis/changelog
};
  ...
  let sql = `
    SELECT id, type, content, context, tier, agent, importance, tags, created_at, last_accessed, access_count, source_session, superseded_by,
      ...
      (
        ...
        + (CASE source_kind
            WHEN 'knowledge_canonical' THEN 0.95
            WHEN 'knowledge_staging' THEN 0.55
            WHEN 'knowledge_generated' THEN 0.45
            WHEN 'consolidated' THEN 1.0
            WHEN 'llm_distilled' THEN 0.75
            WHEN 'tool_verified' THEN 0.70
            WHEN 'user_explicit' THEN 0.65
            WHEN 'agent_inference' THEN 0.30
            WHEN 'markdown_import' THEN 0.20
            ELSE 0.40
          END) * ${w.provenance}
        + (CASE type
            WHEN 'fact' THEN 1.0
            WHEN 'decision' THEN 0.90
            WHEN 'gotcha' THEN 0.85
            WHEN 'learning' THEN 0.45
            WHEN 'note' THEN 0.25
            ELSE 0.40
          END) * ${w.typeSafety}
        + (CASE COALESCE(verification_state, 'hypothesis')
            WHEN 'verified' THEN 1.0
            WHEN 'hypothesis' THEN 0.35
            WHEN 'changelog' THEN 0.20

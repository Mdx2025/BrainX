...
const BRAINX_DIR = path.resolve(__dirname, '..');
const HOOK_PATH = path.join(process.env.HOME || '', '.openclaw', 'hooks', 'brainx-auto-inject', 'handler.js');
...
function checkCronJobs() {
  ...
  try {
    ...
    const detail = `${found}/6 registered, ${enabled} enabled` + (missing.length > 0 ? ` (missing: ${missing.join(', ')})` : '');
